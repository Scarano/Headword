...
import edu.neu.ccs.headword.util.Counter.Count;
...
public class ChannelStats {
	 ...
	 */
	public ChannelStats(int maxSegmentLength) {
	...
	public void addLine(AlignedLine line) {
		for (Segment segment: line.segments) {
			...
			if (charCount == null)
				inputCharCounts.put(segment.input, new Count(1));
			else
				charCount.value++;
			...
			if (charCount == null)
				outputCharCounts.put(segment.output, new Count(1));
			else
				charCount.value++;
			...
			if (outputCount == null)
				outputCounts.put(segment.output, new Count(1));
			else
				outputCount.value++;
		}
	}
	...
	public void computeStats() {
		...
		for (String input: inputCharCounts.keySet())
			inputProbs.put(input, Math.log(inputCharCounts.get(input).value / totalInputChars));

		outputProbs = new HashMap<String, Double>();
		for (String output: outputCharCounts.keySet())
			outputProbs.put(output, Math.log(outputCharCounts.get(output).value / totalInputChars));
		...
		for (String input: segmentCounts.keySet()) {
			...
			for (String output: outputCounts.keySet()) {
				segmentProbs.put(new Segment(input, output),
						Math.log(outputCounts.get(output).value / totalSegments));
			}
		}
	}
...
//alignment.print(System.out);
			...
			ChannelStats model = new ChannelStats(1);
